...
| Matching | Cosine similarity on L2-normalised embeddings, threshold **0.65** |
...
on the JVM (`app/src/test/.../FaceMatcherTest.kt`, 6 tests).
...
- **The 0.65 threshold is set from typical FaceNet behaviour, not tuned on a labelled dataset.**
  Same-person pairs usually score 0.75–0.95 and different people 0.0–0.45, so 0.65 leaves margin
  on both sides — but it has not been validated against a FAR/FRR curve. It is a single constant
  (`FaceMatcher.MATCH_THRESHOLD`) and every screen displays the actual similarity score, so the
  behaviour is easy to inspect and retune.
